...
function ComputeNormals(obj) {
	...
	const nv = obj.vertices.length / 3;
	...
	for(i = 0; i  < nt; ++i){
		const i_v = [obj.triangleIndices[i * 3], obj.triangleIndices[i * 3 + 1], obj.triangleIndices[i * 3 + 2]];

		const p0 = [obj.vertices[3 * i_v[0]], obj.vertices[3 * i_v[0] + 1], obj.vertices[3 * i_v[0] + 2]];
		const p1 = [obj.vertices[3 * i_v[1]], obj.vertices[3 * i_v[1] + 1], obj.vertices[3 * i_v[1] + 2]];
		const p2 = [obj.vertices[3 * i_v[2]], obj.vertices[3 * i_v[2] + 1], obj.vertices[3 * i_v[2] + 2]];
