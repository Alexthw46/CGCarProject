uniformShader = function (gl) {//line 1,Listing 2.14
  ...
  let fragmentShaderSource = `
    ...
    void main(void){                          
    vec3 lightPosVS = vec3(0,0,-7.28) + 2.0 * vLVS;

    if (length(uLightDirection) == 0.0 || uShadingMode == 0){
        gl_FragColor = uColor;	
     }  
        ...
        vec3 L = normalize(lightPosVS - vPosVS);      	
      	...
      	float L_diffuse = max( dot(L,N), 0.0);
        ...
        gl_FragColor = vec4(vShadedColor,1.0);
	 }
